...
//import ScheduleAssignment from '../Schedule';
      ...
      <Box sx={{ display: 'flex' }}>
        ...
        >
          ...
          <List>
            ...
            <ListItem button onClick={() => handleChangeView('scheduleAssignment')}>
              <ListItemText primary="Schedule Assignment" />
            </ListItem>
          </List>
...
function StudentsOverview() {
  ...
  const [timetableSession, setTimetableSession] = useState({session:""});
  const [timetableGroups, setTimetableGroups] = useState([]);
  const [studentsInLevel, setStudentsInLevel] = useState();
  ...
  const handleSubmit = () => {
    if (numGroups !== 0 && subject !== "") {
      axios.post(`${url}/api/assign-timetable`, { timeTableId: timetableSession._id, N_Students: Number(numGroups), ward: subject })
        .then(res => {
  ...
  useEffect(() => {
    if (timetableSession !== "") {
      ...
      //setStudentsInLevel(countStudentsBySpecifiedLevel(selecting, timetableSession.content[0].level))
    }
  }, [timetableSession, update]);

  //function to count students account to their level
  function Students(names, specifiedLevel) {
      ...
      <Box sx={{ maxWidth: 600, mx: 'auto', p: 3, backgroundColor: 'white', borderRadius: 2, boxShadow: 3 }}>
        ...
        <form>
          <TextField
            ...
            onChange={(e) => setTimetableSession(e.target.value)}
            fullWidth
            margin="normal"
          >
